...
The `openapi_metadata` decorator is the primary entry point for adding OpenAPI metadata to API endpoints. It performs several key functions:
...
The `OpenAPISchemaGenerator` class is responsible for generating OpenAPI schemas from Flask-RESTful resources.
...
1. **I18nString**: The `I18nString` class allows you to define strings in multiple languages.
...
5. **I18nBaseModel**: The `I18nBaseModel` class allows you to define Pydantic models with internationalized fields.
...
The `OpenAPIIntegrationMixin` class provides a way to integrate the OpenAPI schema generator with Flask-RESTful's `Api` class.
...
graph TD
    ...
    S[Flask CLI Commands] --> T[Command-Line Interface]
    T --> C
```

## Conclusion

Flask-X-OpenAPI-Schema provides a comprehensive set of components for generating OpenAPI schemas from Flask-RESTful resources. These components work together to provide a seamless integration between Flask, Flask-RESTful, and Pydantic, allowing developers to create well-documented APIs with minimal effort.