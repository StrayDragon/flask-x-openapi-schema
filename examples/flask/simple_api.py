"""
Flask.MethodView 简单 API 示例。

展示如何使用 flask-x-openapi-schema 与 Flask.MethodView 集成。
"""
...
from flask_x_openapi_schema import OpenAPIBlueprintMixin, OpenAPIMethodViewMixin
...
class ItemView(OpenAPIMethodViewMixin, MethodView):
    ...
    def put(self, x_request_path_item_id: str, x_request_body: ItemRequest):
        """更新项目。"""
        # 在实际应用中，这里会更新数据库
        response = ItemResponse(
            id=x_request_path_item_id,
    ...
    def delete(self, x_request_path_item_id: str):
        """删除项目。"""
        # 在实际应用中，这里会从数据库删除数据
        return {"message": f"项目 {x_request_path_item_id} 已删除"}, 200


# 注册视图
ItemsView.register_to_blueprint(api_bp, "/items", endpoint="items")
ItemView.register_to_blueprint(api_bp, "/items/<string:x_request_path_item_id>", endpoint="item")
...
def save_schema():
    ...
    with open(yaml_path, "w") as f:
        f.write(api_bp.generate_openapi_schema(
            title="项目 API",
            version="1.0.0",
            description="用于管理项目的 API",
        ))
    ...
    with open(json_path, "w") as f:
        import json
        json.dump(
            api_bp.generate_openapi_schema(
                title="项目 API",
                version="1.0.0",
                description="用于管理项目的 API",
                output_format="json"
