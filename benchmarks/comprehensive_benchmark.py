...
class VeryComplexModel(BaseModel):
    ...
    id: str = Field(..., description="The ID")
    name: str = Field(..., description="The name")
    items: List[ComplexModel] = Field(default_factory=list, description="List of complex items")
    metadata: Dict[str, Any] = Field(default_factory=dict, description="Metadata")
...
def benchmark_decorator_creation():
    ...
    complex_result = benchmark(
        "Complex decorator creation",
        create_complex_decorator,
        iterations=1000
    )
...
def benchmark_schema_generation():
    ...
    def generate_complex_schema():
        generator = OpenAPISchemaGenerator(
            title="Complex API",
            version="1.0.0",
            description="A more complex API"
        )
        # Register models
    ...
    very_complex_result = benchmark(
        "Very complex schema generation",
        generate_very_complex_schema,
        iterations=1000
    )
...
def run_all_benchmarks():
    ...
    console.print("[bold]Starting comprehensive benchmarks for flask_x_openapi_schema...[/bold]")
    ...
    console.print(create_results_table(
        conversion_results,
        "Pydantic to OpenAPI Schema Conversion"
    ))
    ...
    for i, result in enumerate(slowest_ops, 1):
        console.print(f"{i}. {result.name}: {result.avg_time * 1000:.4f} ms per operation")
    
    # Provide recommendations based on the results
    console.print("\n[bold]Recommendations:[/bold]")
    console.print("1. Consider caching schema generation results for frequently used models")
    console.print("2. Optimize the decorator creation process, especially for complex endpoints")
    console.print("3. Look for opportunities to reduce redundant processing in schema generation")
