...
class TestCommandsCoverage:
    ...
    def test_generate_openapi_command_mdx(self):
        """Test generate_openapi_command with MDX output."""
        # Create a Flask app
        app = Flask(__name__)
        ...
        with runner.isolated_filesystem():
            # Run the command with MDX output within the app context
            with app.app_context():
                ...
                # Check that the MDX output directories were created
                assert os.path.exists("mdx_output/en-US")
                assert os.path.exists("mdx_output/zh-Hans")

                # Check that generate_openapi_schema was called for each language
                assert (
                    api.generate_openapi_schema.call_count == 3
                )  # Once for initial schema, then once for each language
    ...
    def test_generate_openapi_command_with_mdx(self):
        """Test generate_openapi_command with MDX output."""
        # Create a Flask app
        app = Flask(__name__)
        ...
        with runner.isolated_filesystem():
            ...
            with app.app_context():
                ...
                # Check that the MDX output directory was created
                assert os.path.exists("mdx_output")
